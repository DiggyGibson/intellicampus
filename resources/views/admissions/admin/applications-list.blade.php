...
<div class="container-fluid py-4">
            ...
            <div class="d-flex justify-content-between align-items-center">
                ...
                <div class="btn-toolbar" role="toolbar">
                    ...
                    <div class="btn-group" role="group">
                        ...
                        {{-- Note: Applications are created through the public portal, not admin --}}
                        <a href="{{ route('admissions.portal.index') }}" class="btn btn-outline-info" target="_blank">
                            <i class="fas fa-external-link-alt me-1"></i> View Public Portal
                        </a>
                    </div>
                </div>
            </div>
    ...
    <div class="row mb-3">
        <div class="col-12">
            <div class="alert alert-info py-2">
                ...
                @endif
                <a href="{{ route('admin.admissions.applications.index') }}" class="btn btn-sm btn-outline-secondary ms-3">
    ...
    <div class="card shadow">
        <div class="card-body">
            ...
            <div class="row mb-3">
                <div class="col-md-6">
                    <form method="GET" action="{{ route('admin.admissions.applications.index') }}" class="d-flex">
                ...
                <table class="table table-hover" id="applicationsTable">
                    <thead>
                        <tr>
                            ...
                            <th>
                                <a href="{{ route('admin.admissions.applications.index', array_merge(request()->all(), ['sort' => 'application_number', 'order' => request('order') == 'asc' ? 'desc' : 'asc'])) }}">
                                    Application # 
                            ...
                            <th>
                                <a href="{{ route('admin.admissions.applications.index', array_merge(request()->all(), ['sort' => 'last_name', 'order' => request('order') == 'asc' ? 'desc' : 'asc'])) }}">
                                    Applicant Name
                            ...
                            <th>Type</th>
                            <th>
                                <a href="{{ route('admin.admissions.applications.index', array_merge(request()->all(), ['sort' => 'submitted_at', 'order' => request('order') == 'asc' ? 'desc' : 'asc'])) }}">
                                    Submitted
                    ...
                    <tbody>
                        ...
                        <tr class="application-row" data-id="{{ $application->id }}">
                            ...
                            <td>
                                <a href="{{ route('admin.admissions.applications.show', $application->id) }}" class="text-primary">
                            ...
                            <td>
                                ...
                                <br><small class="text-muted">{{ $application->phone_primary ?? $application->phone }}</small>
                            </td>
                            <td>
                            ...
                            <td>
                                @if($application->submitted_at ?? $application->submitted_date)
                                    {{ ($application->submitted_at ?? $application->submitted_date)->format('M d, Y') }}
                                    <br><small class="text-muted">{{ ($application->submitted_at ?? $application->submitted_date)->diffForHumans() }}</small>
                                @else
                            ...
                            <td>
                                @php
                                    $percentage = method_exists($application, 'completionPercentage') ? $application->completionPercentage() : 100;
                                @endphp
                                <div class="progress" style="height: 20px;">
                                    <div class="progress-bar bg-{{ $percentage >= 100 ? 'success' : ($percentage >= 75 ? 'info' : 'warning') }}" 
                                         role="progressbar" 
                            ...
                            <td>
                                ...
                                @if($application->decision)
                                    <br>
                                    <span class="badge bg-{{ $application->decision == 'admitted' ? 'success' : ($application->decision == 'denied' ? 'danger' : 'warning') }} mt-1">
                            ...
                            <td>
                                <div class="btn-group btn-group-sm" role="group">
                                    ...
                                    <a href="{{ route('admin.admissions.applications.edit', $application->id) }}" 
                                    ...
                                    <div class="btn-group btn-group-sm" role="group">
                                        ...
                                        <ul class="dropdown-menu dropdown-menu-end">
                                            ...
                                            <li>
                                                <a class="dropdown-item" href="{{ route('admin.admissions.applications.audit-log', $application->id) }}">
                                            ...
                                            <li>
                                                <form action="{{ route('admin.admissions.applications.destroy', $application->id) }}" method="POST" class="d-inline" onsubmit="return confirm('Are you sure you want to delete this application?');">
                                                    @csrf
                                                    @method('DELETE')
                        ...
                        <tr>
                            <td colspan="11" class="text-center py-4">
                                ...
                                <p class="text-muted">Applications are submitted through the public admissions portal.</p>
                                <a href="{{ route('admissions.portal.index') }}" class="btn btn-primary" target="_blank">
                                    <i class="fas fa-external-link-alt me-1"></i> Go to Admissions Portal
                                </a>
                            </td>
                        </tr>
                        @endforelse
                        ...
                        <div class="card-body">
                            ...
                            <p class="card-text">
                                <small class="text-muted">
                                    ...
                                    <i class="fas fa-phone me-1"></i> {{ $application->phone_primary ?? $application->phone ?? 'N/A' }}<br>
                                    <i class="fas fa-graduation-cap me-1"></i> {{ $application->program->name ?? 'N/A' }}<br>
                                    <i class="fas fa-calendar me-1"></i> {{ ($application->submitted_at ?? $application->submitted_date) ? ($application->submitted_at ?? $application->submitted_date)->format('M d, Y') : 'Not submitted' }}
                                </small>
                            </p>
                            @php
                                $percentage = method_exists($application, 'completionPercentage') ? $application->completionPercentage() : 100;
...
<div class="modal fade" id="filterModal" tabindex="-1">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <form method="GET" action="{{ route('admin.admissions.applications.index') }}">
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-6 mb-3">
                            ...
                            <select name="term_id" class="form-select">
                                ...
                                @foreach($terms ?? [] as $id => $name)
                                    <option value="{{ $id }}" {{ request('term_id') == $id ? 'selected' : '' }}>
                                        {{ $name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="col-md-6 mb-3">
                            ...
                            <select name="application_type" class="form-select">
                                ...
                                <option value="freshman" {{ request('application_type') == 'freshman' ? 'selected' : '' }}>Freshman</option>
                                <option value="transfer" {{ request('application_type') == 'transfer' ? 'selected' : '' }}>Transfer</option>
                                <option value="graduate" {{ request('application_type') == 'graduate' ? 'selected' : '' }}>Graduate</option>
                                <option value="international" {{ request('application_type') == 'international' ? 'selected' : '' }}>International</option>
                            </select>
                        </div>
                        <div class="col-md-6 mb-3">
                ...
                <div class="modal-footer">
                    ...
                    <a href="{{ route('admin.admissions.applications.index') }}" class="btn btn-outline-secondary">Clear Filters</a>
...
<script>
    ...
    function bulkAction(action) {
        ...
        form.method = 'POST';
        form.action = '{{ route("admin.admissions.applications.bulk-action") }}';
    ...
    function exportApplications(format) {
        const url = new URL('{{ route("admin.admissions.applications.export.all") }}');
    ...
    function assignReviewer(id) {
        // Build URL using the base route
        const baseUrl = '{{ url("/admin/admissions/applications") }}';
        window.location.href = baseUrl + '/' + id + '#assign-reviewer';
    }
    ...
    function addNote(id) {
        // You can implement a modal here or use AJAX
        const note = prompt('Enter note:');
        if (note) {
            const baseUrl = '{{ url("/admin/admissions/applications") }}';
            fetch(baseUrl + '/' + id + '/notes', {
                method: 'POST',
                headers: {
                ...
                if (response.ok) {
                    ...
                    location.reload(); // Reload to see the new note
                }
            });
        }
    }
    ...
    function quickAction(id) {
        // Show quick action menu - redirect to show page
        const baseUrl = '{{ url("/admin/admissions/applications") }}';
